...
int bruteForce(Point P[], int n){
    ...
    for (int i = 0; i < n; i++){
        ...
        for (int j = i+1; j < n; j++){
            ...
            if (d < minv)
                minv = d, ptr = (P[i].x < P[j].x ? i : (P[i].y < P[j].y ? i : j));
        }
    }
    res_MIN = min(res_MIN, minv);
    return ptr;
}
...
void find_local_maxs(int xres, int yres, float step) {
...
void reduce_lmaxs(int d){
    int ptr = 0;
    FOR(d){
        ptr = bruteForce(map, map_ptr);
        removeel(ptr);
...
int main() {
    int xres, yres, k, d;
    float step =    scanf("%d%d%d%d", &xres, &yres, &k, &d);
     step = 1.0 / k;

    // initialize_file(yres, xres);
    
    find_local_maxs(xres, yres, step);
    ...
    FOR(map_ptr){
        FOR(j,map_ptr){
            if(i != j)            res_MAX=max(res_MAX,dist(map[i],map[j]));
        }
    }
