...
; zmiana let-lex-addr.rkt, rozszerzenie o lambdy i aplikacje
...
(define (translate [e : Exp] [env : (EnvA Symbol)]) : ExpA
  (type-case Exp e
    ...
    [(appE e1 e2)
     (letA (translate e2 env)
           (type-case Exp e1
             [(lamE x e)
              (translate e (extend-envA env x))]
             [else
              (error 'translate "Application on non-funtion expression")]))]
    [else (error 'translate "Wrong expression")]))
