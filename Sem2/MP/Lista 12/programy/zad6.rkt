...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(letmutrecE x1 e1 x2 e2 e)
     (letrec ([new-env (extend-env-undef (extend-env-undef env x1) x2)]    ; to extend to list we could simply recursively add variables to env
              [v1 (eval e1 new-env)]  ; and here recursively calculate their value
              [v2 (eval e2 new-env)])
       (begin (update-env! new-env x1 v1)   ; and recursively update the values
