...
(define (rec-string-append [xs : (Listof String)]) : String
    (cond 
        [(empty? xs) ""]
        [(empty? (rest xs))
            (first xs)]
        [else
            (string-append
                (first xs)
                (rec-string-append (rest xs)))]))

;? rough idea: we simply treat (state -> ('a * state))  as ('a * state) using lambda to produce
;? it from our monad, res is quite intuitive
...
(define (returnM [v : 'a]) : (M 'a)
  (λ ([s : State]) (valE (pair v s))))

(define (bindM [c : (M 'a)] [f : ('a -> (M 'b))]) : (M 'b)
  (λ ([s : State])
    (let ([pM (c s)])
         (type-case (E ('a * State)) pM
           [(valE p)
            ((f (fst p)) (snd p))]
           [(errE l m)
            (errE l m)]))))
...
(define (showM [c : (M 'a)]) : String
  (let ([startM (c 0)])
    (type-case (E (Value * State)) startM
      [(valE p)
        (rec-string-append (list
          "value: "   (value->string (fst p)) 
          ", state: " (to-string (snd p))))]
      [(errE l m) (rec-string-append (list
                "error in " (to-string l) ": " m))])))
    ...
    [(opE o e1 e2)
     (bindM (eval e1 env)
        (λ (v1) (bindM (eval e2 env)
            (λ (v2) (bindM tickM          ; we take the state+1 module from tick
                (λ (v3) ((op->proc o) v1 v2)))))))] ; and ignore its (void) value
    [(varE x)
     (lookup-env x env)]
    [(lamE x b)
