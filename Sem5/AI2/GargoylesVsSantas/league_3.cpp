...
public:
    ...
    int greedy_fireball() const {
        // throw fireball at the present that is closest to the opponent, and opponent is closer to it than we are
        int best = INT_MAX;
        int best_id = -1;

        for(const Present &p : presents) {
            const int my_dist = my_gargoyle.pos.int_dist(p.pos);
            if(my_dist > 300*300) continue; // too far

            const int opp_dist = opp_gargoyle.pos.int_dist(p.pos);

            if(opp_dist < my_dist && opp_dist < best) {
                best = opp_dist;
                best_id = p.id;
            }
        }
