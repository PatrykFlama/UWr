...
public:
    ...
    void hide_harvesters() {
        for(auto& [pos, organ] : my_organs) {
            if(organ.type == MY_HARVESTER) {
                const Point new_p = pos + DIRS[organ.direction];
                grid[new_p.y][new_p.x] = WALL;
    ...
    pair<int, Point> sporers_can_spore() {
        ...
        for(auto& [pos, organ] : my_organs) {
            ...
            if(organ.type == MY_SPORER) {
                Point new_p = pos + DIRS[organ.direction];
                while(safe(new_p) && (grid[new_p.y][new_p.x] == EMPTY || proteins.find(new_p) != proteins.end())) {
                    if(proteins.find(new_p) != proteins.end()) {
    ...
    pair<Point, DIR> plant_sporers() {
        ...
        for(const auto& [pos, organ] : my_organs) {
            ...
            for(int i = 0; i < 4; i++) {
                ...
                if(!safe(new_p) || (grid[new_p.y][new_p.x] != EMPTY && proteins.find(new_p) == proteins.end())) continue;

                for(int j = 0; j < 4; j++) {
                    Point new_new_p = new_p;

                    while(safe(new_new_p) && grid[new_new_p.y][new_new_p.x] == EMPTY) {
...
public:
    ...
    int get_parent_id(const Point &pos) {
        ...
        for(int dir = 0; dir < 4; dir++) {
            ...
            if(safe(new_p) && state.my_organs.find(new_p) != state.my_organs.end()) {
