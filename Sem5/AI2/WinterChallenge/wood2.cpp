    ...
    while(1) {
        ...
        for (int i = 0; i < required_actions_count; i++) {
            ...
            if(move == Point(-1, -1)) {
                ...
                auto [pos, dir] = ai.state.opp_is_neigh();

                if(pos != Point(-1, -1) && ai.state.my_proteins_cnt[1] > 0 && ai.state.my_proteins_cnt[2] > 0) {
                    cout << "GROW " << ai.get_parent_id(move) << ' ' << move << " TENTACLE " << DIR_TO_GEO[dir] << endl;
