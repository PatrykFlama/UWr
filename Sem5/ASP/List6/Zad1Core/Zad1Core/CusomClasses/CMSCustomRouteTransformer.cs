...
namespace Zad1Core.CusomClasses
{
        ...
        public override async ValueTask<RouteValueDictionary> TransformAsync(HttpContext httpContext, RouteValueDictionary values)
        {
            ...
            if(segments.Length >= 0)
            {
                if(segments.Last().IndexOf(DEFAULTPAGEEXTENSION) > 0)
                {
                    ...
                    values[SITENAME] = string.Join("/", segments.Take(segments.Length - 1).ToArray()); values[PAGENAME] = segments.Last().Substring(0, segments.Last().IndexOf("."));
