...
class SubtreeSum {
   public:
    ...
    SubtreeSum(int n, const vector<int>& _values)
    ...
    void build() {
        ...
        vector<SegmentTree::Node> initialSegValues;
        for (int node : eulerTour) {
            initialSegValues.push_back(SegmentTree::Node(values[node]));
        }
    ...
    int query(int v) {
        return segTree.query(firstVisit[v], lastVisit[v]).val;
    }
};
